        ...
        if action == 'start':
            ...
            try:
                ...
                if melotts_action == 'start':
                    # 启动MeloTTS服务
                    controller.config['use_molotts'] = True
                    controller.save_config()
                    controller.add_log_message('MeloTTS 服务已启用', 'INFO')
                    socketio.emit('config_updated', controller.config)
                    return jsonify({'success': True, 'message': 'MeloTTS 服务已启用'})

                elif melotts_action == 'stop':
                    # 停止MeloTTS服务
                    controller.config['use_molotts'] = False
                    controller.save_config()
                    controller.add_log_message('MeloTTS 服务已禁用', 'INFO')
                    socketio.emit('config_updated', controller.config)
                    return jsonify({'success': True, 'message': 'MeloTTS 服务已禁用'})

                elif melotts_action == 'warmup':
                    # 预热MeloTTS模型
                    controller.add_log_message('开始预热MeloTTS模型...', 'INFO')
                    # 这里可以添加实际的预热逻辑
                    # 比如发送一个测试请求来加载模型到内存
                    try:
                        import requests
                        import json
                        api_url = controller.config.get('melotts_api_url', 'http://127.0.0.1:8000')
                        test_url = f"{api_url}/tts"

                        # 发送一个简短的测试请求来预热模型
                        test_data = {
                            "text": "Test",
                            "speaker_id": int(controller.config.get('molotts_speaker_id', 0)),
                            "sdp_ratio": float(controller.config.get('molotts_sdp_ratio', 0.2)),
                            "noise_scale": float(controller.config.get('molotts_noise_scale', 0.6)),
                            "noise_scale_w": float(controller.config.get('molotts_noise_scale_w', 0.8)),
                            "speed": float(controller.config.get('molotts_speed', 1.0))
                        }

                        response = requests.post(test_url, json=test_data, timeout=30)
                        if response.status_code == 200:
                            controller.add_log_message('MeloTTS 模型预热成功', 'INFO')
                            return jsonify({'success': True, 'message': 'MeloTTS 模型预热成功'})
                        else:
                            controller.add_log_message(f'MeloTTS 模型预热请求失败，状态码: {response.status_code}', 'WARNING')
                            return jsonify({'success': True, 'message': f'MeloTTS 预热请求完成，状态码: {response.status_code}'})
                    except Exception as e:
                        controller.add_log_message(f'MeloTTS 模型预热失败: {str(e)}', 'WARNING')
                        return jsonify({'success': False, 'message': f'MeloTTS 预热失败: {str(e)}'})

                else:
                    return jsonify({'success': False, 'message': f'未知的MeloTTS操作: {melotts_action}'})

            except Exception as e:
                logger.error(f"Error controlling MeloTTS: {e}")
                controller.add_log_message(f'控制MeloTTS服务失败: {str(e)}', 'ERROR')
                return jsonify({'success': False, 'message': f'控制MeloTTS服务失败: {str(e)}'})
